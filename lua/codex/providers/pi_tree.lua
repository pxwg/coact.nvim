...
local ns = vim.api.nvim_create_namespace("codex.nvim.pi_tree")
...
local function entry_segments(state, node)
  ...
  if node.label then
    append_segment(segments, "[" .. tostring(node.label) .. "] ", "CodexPiTreeLabel")
  end
  if e.type == "message" then
    local message = as_table(e.message)
    local role = tostring(message.role or "message")
    if role == "user" then
      append_segment(segments, "user: ", "CodexPiTreeUser")
      append_segment(segments, truncate(normalize_text(text_content(message.content, 200)), 200), "CodexPiTreeText")
    elseif role == "assistant" then
      append_segment(segments, "assistant: ", "CodexPiTreeAssistant")
      local text = truncate(normalize_text(text_content(message.content, 200)), 200)
      if text ~= "" then
        append_segment(segments, text, "CodexPiTreeText")
      elseif message.stopReason == "aborted" then
        append_segment(segments, "(aborted)", "CodexPiTreeMuted")
      elseif message.errorMessage then
        append_segment(segments, truncate(normalize_text(message.errorMessage), 80), "CodexPiTreeError")
      else
        append_segment(segments, "(no content)", "CodexPiTreeMuted")
      end
    elseif role == "toolResult" then
      ...
      if tool_call then
        append_segment(segments, format_tool_call(tool_call.name, tool_call.arguments), "CodexPiTreeTool")
      else
        append_segment(segments, "[" .. tostring(message.toolName or "tool") .. "]", "CodexPiTreeTool")
      end
    elseif role == "bashExecution" then
      append_segment(segments, "[bash]: ", "CodexPiTreeTool")
      append_segment(segments, truncate(normalize_text(message.command or ""), 120), "CodexPiTreeMuted")
    else
      append_segment(segments, "[" .. role .. "]", "CodexPiTreeMuted")
    end
  elseif e.type == "custom_message" then
    append_segment(segments, "[" .. tostring(e.customType or "custom") .. "]: ", "CodexPiTreeCustom")
    append_segment(segments, truncate(normalize_text(text_content(e.content, 200)), 200), "CodexPiTreeText")
  elseif e.type == "branch_summary" then
    append_segment(segments, "[branch summary]: ", "CodexPiTreeBranchSummary")
    append_segment(segments, truncate(normalize_text(e.summary or ""), 200), "CodexPiTreeText")
  elseif e.type == "compaction" then
    append_segment(
      segments,
      ("[compaction: %dk tokens]"):format(math.floor((tonumber(e.tokensBefore) or 0) / 1000 + 0.5)),
      "CodexPiTreeCompaction"
    )
  elseif e.type == "model_change" then
    append_segment(segments, "[model: " .. tostring(e.modelId or "") .. "]", "CodexPiTreeMuted")
  elseif e.type == "thinking_level_change" then
    append_segment(segments, "[thinking: " .. tostring(e.thinkingLevel or "") .. "]", "CodexPiTreeMuted")
  elseif e.type == "session_info" then
    append_segment(segments, "[title: " .. tostring(e.name or "empty") .. "]", "CodexPiTreeMuted")
  elseif e.type == "label" then
    append_segment(segments, "[label: " .. tostring(e.label or "(cleared)") .. "]", "CodexPiTreeMuted")
  else
    append_segment(segments, "[" .. tostring(e.type or "entry") .. "]", "CodexPiTreeMuted")
  end
  return segments
end
...
local function render_model(state, width, height)
  ...
  add_plain("Session Tree" .. status_labels(state), "CodexPiTreeTitle")
  add_plain(
    "j/k ctrl-n/p move · ctrl-d/u half-page · ctrl-f/b page · gg/G edge · h/l branch · d/t/u/L/a filters · / search",
    "CodexPiTreeHelp"
  )
  add_plain("Type / to search: " .. tostring(state.search_query or ""), "CodexPiTreeHelp")
  add_plain(string.rep("─", math.max(1, width)), "CodexPiTreeConnector")

  if #state.filtered_nodes == 0 then
    add_plain("  No entries found", "CodexPiTreeMuted")
    add_plain("  (0/0)" .. status_labels(state), "CodexPiTreeHelp")
  ...
  for index = start_index, end_index do
    ...
    append_segment(line, is_selected and "› " or "  ", is_selected and "CodexPiTreeCursor" or nil)
    append_segment(line, prefix_text(state, flat_node), "CodexPiTreeConnector")
    append_segment(
      line,
      id and state.active_path_ids[id] and "• " or "  ",
      id and state.active_path_ids[id] and "CodexPiTreeActive" or nil
    )
    ...
    if is_selected then
      selected_row = #lines
      line_hls[#lines] = "CodexPiTreeSelected"
    end
  end
  add_plain(("(%d/%d)%s"):format(state.selected_index, #state.filtered_nodes, status_labels(state)), "CodexPiTreeHelp")
  return {
    lines = lines,
    spans = spans,
    line_hls = line_hls,
...
local function setup_highlights()
  vim.api.nvim_set_hl(0, "CodexPiTreeTitle", { default = true, link = "Title" })
  vim.api.nvim_set_hl(0, "CodexPiTreeHelp", { default = true, link = "Comment" })
  vim.api.nvim_set_hl(0, "CodexPiTreeConnector", { default = true, link = "LineNr" })
  vim.api.nvim_set_hl(0, "CodexPiTreeCursor", { default = true, link = "Identifier" })
  vim.api.nvim_set_hl(0, "CodexPiTreeActive", { default = true, link = "Identifier" })
  vim.api.nvim_set_hl(0, "CodexPiTreeSelected", { default = true, link = "Visual" })
  vim.api.nvim_set_hl(0, "CodexPiTreeUser", { default = true, link = "Identifier" })
  vim.api.nvim_set_hl(0, "CodexPiTreeAssistant", { default = true, link = "DiagnosticOk" })
  vim.api.nvim_set_hl(0, "CodexPiTreeTool", { default = true, link = "Comment" })
  vim.api.nvim_set_hl(0, "CodexPiTreeMuted", { default = true, link = "Comment" })
  vim.api.nvim_set_hl(0, "CodexPiTreeText", { default = true, link = "Normal" })
  vim.api.nvim_set_hl(0, "CodexPiTreeLabel", { default = true, link = "DiagnosticWarn" })
  vim.api.nvim_set_hl(0, "CodexPiTreeCustom", { default = true, link = "Special" })
  vim.api.nvim_set_hl(0, "CodexPiTreeBranchSummary", { default = true, link = "DiagnosticWarn" })
  vim.api.nvim_set_hl(0, "CodexPiTreeCompaction", { default = true, link = "Special" })
  vim.api.nvim_set_hl(0, "CodexPiTreeError", { default = true, link = "DiagnosticError" })
end
...
function M.is_request(message)
  ...
  return type(options[1]) == "table" and options[1].__codexNvimPiTree == true
end
...
function M.select(message, callback)
  ...
  vim.bo[bufnr].filetype = "codex-pi-tree"
