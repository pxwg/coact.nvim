...
function assertBridgeEnv() {
  const addr = process.env.CODEX_NVIM_PI_EDIT_BRIDGE_ADDR;
  const nonce = process.env.CODEX_NVIM_PI_EDIT_BRIDGE_NONCE;
  const nvim = process.env.CODEX_NVIM_PI_EDIT_BRIDGE_NVIM || "nvim";
  if (!addr) {
    throw new Error("codex.nvim Pi edit bridge is not connected to a Neovim RPC server.");
  }
  if (!nonce) {
    throw new Error("codex.nvim Pi edit bridge is missing its Neovim nonce.");
  }
...
async function reviewWithNeovim(payload, signal) {
  ...
  const dir = await mkdtemp(join(tmpdir(), "codex-nvim-pi-edit-"));
  const payloadPath = join(dir, "payload.json");
  const resultPath = join(dir, "result.json");
  const argsJson = JSON.stringify({ payload: payloadPath, nonce, result: resultPath });
  const expr =
    "luaeval('require(\"codex.providers.pi_edit_bridge\").review_file_async(_A)', " + luaSingleQuote(argsJson) + ")";
  ...
  try {
    ...
    const configured = Number(process.env.CODEX_NVIM_PI_EDIT_BRIDGE_TIMEOUT_MS || "600000");
...
export default function (pi) {
  ...
  pi.registerTool({
    ...
    label: "edit",
    description:
      "Edit a single file using exact text replacement. codex.nvim opens the proposed changes in Neovim for interactive approval before anything is written.",
    promptSnippet: "Make precise file edits with exact text replacement, reviewed by codex.nvim in Neovim before write",
    promptGuidelines: [
  ...
  pi.registerTool({
    ...
    label: "write",
    description:
      "Write content to a file. codex.nvim opens the proposed create/overwrite change in Neovim for interactive approval before anything is written.",
    promptSnippet: "Create or overwrite files, reviewed by codex.nvim in Neovim before write",
...
local function ensure_extension_path()
  ...
  local path = vim.fn.tempname() .. "-codex-nvim-pi-edit-bridge.ts"
...
function M.prepare_command(command, env)
  ...
  env = env or {}
  env.CODEX_NVIM_PI_EDIT_BRIDGE_ADDR = address
  env.CODEX_NVIM_PI_EDIT_BRIDGE_NONCE = ensure_nonce()
  env.CODEX_NVIM_PI_EDIT_BRIDGE_NVIM = ensure_nvim_bin()
  env.CODEX_NVIM_PI_EDIT_BRIDGE_TIMEOUT_MS = tostring(timeout_ms())
...
local function payload_thread_id(payload)
  local ok, pi = pcall(require, "codex.providers.pi")
...
function M.review_payload_async(payload, done)
  ...
  vim.schedule(function()
    local session, open_err = require("codex.patch_session").open({
