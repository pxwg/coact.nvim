...
function M.command(message)
  ...
  local title = params.command and ("Run command: " .. params.command) or "Approve Codex command"
...
function M.permissions(message)
  local rpc = require("codex.rpc")
  choose("Approve Codex permission request", { "accept", "decline", "cancel" }, function(decision)
    rpc.respond(message.id, { decision = decision })
  end)
end
