...
local group = vim.api.nvim_create_augroup("codex.nvim.behavior", { clear = true })
...
local function is_trackable_buffer(bufnr, cwd)
  ...
  if context.is_codex_buffer(bufnr) then
    return false
  end
...
local function anchor_dir(anchor)
  ...
  return vim.fs.joinpath(state_dir, "codex.nvim", "behavior", workspace, thread)
end
