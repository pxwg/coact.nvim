...
local noop_marker_prefix = ".coact-nvim-apply-patch-noop-"
...
local function hook_script_path()
  return vim.fs.joinpath(plugin_root(), "scripts", "coact-nvim-apply-patch-hook")
end

local function debug_log_path()
  local from_global = vim.g.coact_native_apply_patch_debug_log
  if type(from_global) == "string" and from_global ~= "" then
    return from_global
  end
  local from_env = vim.env.COACT_NVIM_APPLY_PATCH_DEBUG_LOG
  ...
  if opts.debug == true then
    return vim.fs.joinpath(trim_trailing_slash(vim.env.TMPDIR or "/tmp"), "coact-nvim-apply-patch-debug.log")
  end
  return nil
end
...
function M.enable_debug(path)
  if type(path) ~= "string" or path == "" then
    path = vim.fs.joinpath(trim_trailing_slash(vim.env.TMPDIR or "/tmp"), "coact-nvim-apply-patch-debug.log")
  end
  vim.g.coact_native_apply_patch_debug_log = path
...
local function setup_cleanup_autocmd()
  cleanup_augroup = vim.api.nvim_create_augroup("CoactNvimNativeApplyPatchHook", { clear = true })
...
function M.prepare_app_server(command, env)
  ...
  env = env or {}
  env.COACT_NVIM_LISTEN_ADDRESS = address
  env.COACT_NVIM_HOOK_NONCE = command_nonce
  env.COACT_NVIM_BIN = command_nvim_bin
  local debug_path = debug_log_path()
  if debug_path then
    env.COACT_NVIM_APPLY_PATCH_DEBUG_LOG = debug_path
  end
...
local function validate_patch(cwd, patch)
  local changes, err = require("coact.dynamic_tools")._changes_from_native_apply_patch(cwd, patch, {
    allow_absolute = true,
  })
  if not changes then
    return nil, err
...
local function noop_patch(cwd, item_id)
  ...
  local ok, err = pcall(vim.fn.writefile, {
    "coact.nvim native apply_patch completion marker",
  }, absolute)
  if not ok then
...
function M.review_payload(payload)
  ...
  if util.value(payload.tool_name) ~= "apply_patch" then
    ...
    return deny_output("coact.nvim native apply_patch hook received unsupported tool: " .. tostring(payload.tool_name))
  end
  local cwd = util.value(payload.cwd) or config.cwd()
  local changes
...
function M.review_payload_async(payload, done)
  ...
  if util.value(payload.tool_name) ~= "apply_patch" then
    M.debug_log("review_payload_unsupported_tool", { tool_name = util.value(payload.tool_name) })
    done(deny_output("coact.nvim native apply_patch hook received unsupported tool: " .. tostring(payload.tool_name)))
    return
  end
  local cwd = util.value(payload.cwd) or config.cwd()
  local changes
